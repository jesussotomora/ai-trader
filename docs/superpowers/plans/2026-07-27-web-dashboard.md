# Web Dashboard & FastAPI UI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement a sleek, modern FastAPI web dashboard (`src/web/app.py`) for `ai-trader` providing real-time UI visualization for model leaderboards, interactive backtest simulations, subscription prompt generation, and portfolio tracking.

**Architecture:** FastAPI backend serving REST API endpoints and a single-page HTML5/CSS3 glassmorphism dashboard (`src/web/templates/index.html`).

**Tech Stack:** Python 3.9+, FastAPI, uvicorn, HTML5, Vanilla CSS, JavaScript, pytest.

## Global Constraints

- Must follow modern web design guidelines (dark mode, glassmorphism, vibrant accent colors, Google Fonts Inter).
- FastAPI endpoints must be covered with unit tests using `fastapi.testclient.TestClient`.
- Pure frontend without external heavy node dependencies.

---

### Task 1: FastAPI App & REST API Endpoints
...
**Interfaces:**
- Consumes: `fastapi`, `src/engine/arena.py`, `src/engine/backtest.py`, `src/compliance/cli_assistant.py`
- Produces: `GET /api/leaderboard`, `POST /api/backtest`, `POST /api/prompt`, `GET /api/portfolio`

- [ ] **Step 1: Write failing FastAPI web app tests**
...
Run: `./.venv/bin/pytest tests/test_web_app.py`  
Expected: FAIL with `ModuleNotFoundError: No module named 'src.web.app'`

- [ ] **Step 3: Implement FastAPI app in `src/web/app.py`**
...
- [ ] **Step 5: Commit FastAPI web app**

```bash
git add src/web/__init__.py src/web/app.py tests/test_web_app.py
git commit -m "feat: implement FastAPI web dashboard with glassmorphism UI, leaderboard API, and backtest runner"
```

---

### Task 2: Master Test Suite Verification

**Files:**
- Modify: `tests/test_integration.py`

- [ ] **Step 1: Add web app endpoint integration test**
...
Expected: PASS (all 28+ tests pass across schemas, scraper, compliance, risk, arena, council, backtest, alpaca, market_data, cli_assistant, web_app)

- [ ] **Step 3: Commit integration update**

```bash
git add tests/test_integration.py
git commit -m "test: add FastAPI web app integration test to master test suite"
```
